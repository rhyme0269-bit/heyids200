...
## [docs: 後台使用手冊與文件同步近期變更] - 2026-08-26T08:00:00Z

### 變更類型

文件（後台使用手冊、聯絡表單設定、疑難排解）

### 起因

客戶已把 Apps Script 收件網址填入基本資訊，但線上仍看不到表單。查證後確認線上聯絡頁仍嵌著 Google 表單（`forms.gle/...`），`googleFormUrl` 一有值就優先，`contactEndpoint` 那行程式碼根本不會執行。

而**我給他看的文件裡沒有這一步** —— `docs/聯絡表單設定.md` 提到收件網址三次，卻從未提到要清空 Google 表單網址。文件缺漏直接造成客戶卡了數日。

因此順帶全面盤點手冊與文件是否跟得上近期變更。

### 盤點結果：缺漏比預期嚴重

以關鍵字檢查手冊，命中次數為 0 的項目：`配色`、`Instagram`、`收件網址`、`Apps`、`營業時間`、`累積案件`、`備份`、`publish`。

**最嚴重的是 `publish` 為 0** —— 手冊完全沒有提到「後台儲存不等於線上更新」。而 `overview-intro` 原本寫著：

> 大部分修改都會即時反映在網站前台。

在目前架構下這句是**錯的**：後台跑在客戶自己電腦上，線上網站只有執行 `npm run publish` 才會變。「快速入門」的流程也在「預覽確認」就結束，沒有發布這一步。

### 手冊修正

| 章節 | 處理 |
|---|---|
| `overview-intro` | 移除「即時反映在前台」的錯誤敘述 |
| **`overview-publish`（新增）** | 編輯→發布的兩段式架構、四步流程、常見誤會 |
| `overview-login` | 補上帳密來自 `.env`、改完要重啟（#39）、無 `.env` 時預設帳密一直有效 |
| `pages-icons` | 全面改寫：原本教「輸入 emoji」並附 emoji 對應表，已改為圖示庫選取（#32） |
| **`pages-contact-form`（新增）** | 兩種收件做法對照表、必須先清空 Google 表單網址、實測步驟 |
| `pages-google-form` | 改名「改用 Google 表單」，補上會覆蓋內建表單、以及需登入 Google 的限制 |
| `pages-links` | 更新可點擊卡片的外觀說明（#41），補上站內／站外網址的差異 |
| `pages-blocks` | 「項目列表」不再說「可加 emoji 圖示」；「聯絡雙欄」註明地圖跟著地址 |
| `settings-edit` | 欄位清單補齊 Instagram／收件網址／營業時間／累積案件／配色；**修正錯誤敘述** |
| **`settings-theme`（新增）** | 網站配色五個顏色的說明與注意事項 |
| `ts-common` | 「儲存後內容沒更新」的排查方向已過時，改為以未發布為首要原因；新增 5 列 |
| 快速入門時間軸 | 新增「設定表單收件」與「發布上線」兩步 |
| 注意事項 | 首條改為發布提醒 |
| 快速索引 | 新增發布、表單收件、配色、改帳密四個入口 |

**修正的錯誤敘述**：`settings-edit` 原本寫「電子郵件 — 訪客聯絡表單的收件地址」。實際上該欄位只是顯示給訪客看的聯絡信箱，表單收件由「聯絡表單收件網址」決定。

版本號 v2.0 → v2.1。

### 文件修正

**`docs/聯絡表單設定.md`** —— 新增「步驟 4：確認沒有嵌入 Google 表單」（原步驟 4、5 順延為 5、6），附兩種做法的對照表；步驟 6 的測試清單加入「畫面上看到的是四個欄位而不是嵌入表單」。

**`docs/疑難排解.md`** —— 三處：

- 「忘記後台密碼」**原本是過時的**：寫著「若修改 `.env` 後仍無法登入，是因為帳號在首次啟動時就已建立。請聯繫我們協助重設。」#39 修好後改 `.env` 重啟即生效，已改寫為正確步驟
- 新增「找不到後台網址 / 連不上 localhost:8081」（無 `.env` 時連接埠為 80，#36）
- 新增「聯絡頁沒有出現表單欄位」（清空 Google 表單網址）

### 驗證

後台手冊需登入且要點選頁籤才看得到，本機無瀏覽器擴充可自動操作，故改以 `react-dom/server` 直接渲染該元件：渲染成功（15 KB），逐字檢視快速入門流程、快速索引與章節清單皆正確。

`buildSections` 的章節 id 共 33 個，快速索引 21 個 target **無指向不存在的 id**。殘留 emoji 教學字樣 0 處。`npm run build` 通過。

## [modify: 卡片右上角改回項目編號（#41 追加）] - 2026-08-26T06:00:00Z
...
> 卡片下方已有「查看詳細 →」／「前往網站 ↗」，那右上角箭頭幫我改回項目編號就好

底部已有動作標籤，右上角再放一個箭頭等於同一件事講兩次。

### 修正範圍的判斷

站外連結的 `↗` **並非本次新增** —— 原本 `item.url && !isInternal ? "↗" : 編號` 就是這樣，實用連結頁一直顯示 `↗`。#41 新增的只有站內連結的 `→`。

客戶看的是服務項目頁，因此「改回項目編號」精確對應的是把站內連結那格改回編號；站外的 `↗` 維持原樣不動，它區分的是「會離開網站」而不是「這是連結」，且動它等於做客戶沒要求的變更。

角落顏色一併還原為 `text-stone-300 group-hover:text-amber-700`（原始樣式），不再對可點卡片另外上色。

### 可點擊的辨識仍然成立

移除角落箭頭後，可點卡片仍有：較深邊框、琥珀色圖示磚、底部動作標籤、hover 浮起與底線掃過、`focus-visible` 外框。
...
`/links` 與變更前一致，未受影響。`npm run build` 通過。

佐證：`screenshots/services-corner-numbers.jpg`

## [feat: 套用線條圖示、可點擊卡片加上靜態辨識（#32, #41）] - 2026-08-25T11:00:00Z
...
服務項目的項目列表，若有連結到其他頁面，希望前端有區別讓使用者知道可以點擊。

### 問題比回報的更嚴重

`/services` 九項服務中**只有一項**（不動產買賣移轉登記 → `/flow-sale`）有連結，但九張卡片外觀完全相同：同樣邊框、同樣右上角編號、**同樣的 hover 浮起效果**。

最後一項是反向誤導 —— 不可點的卡片也會浮起，本身就在暗示可點；而觸控裝置沒有 hover，連這個錯誤的提示都看不到。等於原本的可點性提示不只是不足，是錯的。
...
| | 可點卡片 | 不可點卡片 |
|---|---|---|
| 邊框 | `border-stone-300`（較深） | `border-stone-200/70` |
| 右上角 | `→`（站內）／`↗`（外部），琥珀色 | 項目編號，淺灰 |
| 卡片底部 | 「查看詳細 →」／「前往網站 ↗」 | 無 |
| 圖示磚 | 琥珀底＋琥珀圖示 | 灰底＋灰圖示 |
| hover 浮起與底線掃過 | 有 | **移除** |
| 鍵盤焦點 | `focus-visible` 琥珀色外框 | 不適用 |

### 連帶修正：`/services` 的小標籤顯示「LINKS」

`KeyValueListRenderer` 的判斷是 `items.some(item => item.url)` —— 只要**任一項**有連結就整區顯示 "Links"。買賣移轉取得 `/flow-sale` 連結後（先前為 #23／#24 所加），整個服務項目區塊的標籤就翻成了 "LINKS"。客戶未回報，但明顯錯誤。

改為 `items.every(item => item.url && !item.url.startsWith("/"))` —— 全部項目都連往站外才算連結目錄。`/services` 與首頁回到 "SERVICES"，`/links` 三個區塊維持 "LINKS"。
...
`ServiceIcon`（12 個圖示）與 `IconPicker` 已存在且已接入後台，但**前台沒有使用** —— `ServiceCard` 仍直接輸出原始 emoji。且 `IconPicker` 是純 emoji 選擇器，與這組線條圖示無關。

### 新增三個圖示

實用連結原有 8 筆若只用既有 12 個圖示會過度重複，且台電會配到水滴：

| 代號 | 圖形 | 用於 |
|---|---|---|
| `power` | 閃電 | 台電過戶（與 `utility` 水滴分開） |
| `search` | 放大鏡 | 地政士資料查詢 |
| `registry` | 登錄簿 | 內政部實價登錄查詢 |

共 15 個代號，**全部都有實際使用**。

### 資料遷移（關鍵）

種子值早已寫入每個安裝的 block 資料，因此**只改種子對客戶完全無效**。新增 `ICON_MIGRATION`，沿用既有 url 回填的模式：逐項比對、只換我們種下的 emoji、每次啟動都可安全執行（換完是代號，不再匹配）。

**按頁面分開對照**，因為同一 emoji 在不同頁意義不同：🏠 在 `/services` 是買賣移轉，在 `/links` 是實價登錄 —— 單一對照表會讓實價登錄配到房子圖示。

### 後台選擇器

`IconPicker` 加入「事務所圖示（建議）」區塊置於最上方，附中文名稱；emoji 保留於下方。觸發按鈕改用 `ServiceIcon` 顯示目前值，故代號顯示 SVG、emoji 原樣顯示。

### 自行發現並修正：emoji 會被幾何類別壓變形

`ServiceIcon` 原本把 `className` 同時套用於 SVG 與 emoji 分支。呼叫端傳的是 `h-6 w-6`（寬高），套在 emoji 這個**文字**上會以幾何方式裁切字形。種子資料現已全為代號，但選擇器仍提供 emoji，這條路徑依然存在。已將 emoji 分支改用獨立的 `emojiClassName`（預設 `text-xl`），不再繼承 `className`。

實測：暫時把「繼承登記」改回 🌳，與相鄰的 SVG 圖示並排放大比對，尺寸與位置一致；測後已還原。

### 驗證

| 項目 | 結果 |
|---|---|
| emoji → 代號遷移 | 26 項全部轉換，殘留 0 |
| 動作標籤數（`class` 精確計數） | `/services` 1／9、`/links` 8／8、首頁 1／9 |
| 小標籤 | `/services`、首頁 = SERVICES；`/links` 三區塊 = LINKS |
| 15 個圖示 24px 辨識度 | 放大檢視全部可辨 |
| emoji fallback | 與 SVG 同尺寸同位置 |
| RWD 375／768px | 單欄／雙欄正確，可點卡片在兩種寬度都可辨 |
| `npm run build` | 通過 |

佐證：`screenshots/services-icons-clickable.jpg`、`links-icons-power-water.jpg`、`services-clickable-rwd.jpg`、`icon-set-zoom.jpg`

> 計數註記：直接 grep 頁面 HTML 會得到兩倍數字 —— App Router 會把 RSC payload 內嵌為 script，同一字串出現兩次。上表以 class 屬性精確計數。
...
### Issue #30：Instagram 連結

客戶希望在網站加入 Instagram（事務所最新消息透過 IG 宣傳）。

做的是**連結**而非嵌入貼文牆 —— 嵌入需串接 Meta 官方介面或第三方套件，與規格「不要為了視覺效果大量增加第三方套件」衝突。

`instagramUrl` 設為 settings 而非寫死：客戶把帳號網址填進後台即會出現於頁尾與聯絡我們頁，**不需再開發**，也避免我們編造帳號。欄位為空時完全不渲染，因此先行上線不會留下空白區塊。

驗證：未設定時首頁與聯絡頁皆為 0 處；透過後台 API 寫入後兩處皆出現；測試後已清空。

### P1：小工具、FAQ、實用連結

**小工具（第十九節，規格標為「重要轉換入口」）**

- 所有 input／select 統一 48px 高與一致的 focus 樣式；按鈕 52px、12px 圓角，與 CTA 系統對齊
- 結果區改為帶邊框的淡金底，比其上方的欄位更容易辨識
- 試算結果下方新增「對試算結果有疑問？立即諮詢地政士 →」。置於數值**下方**而非覆蓋其上 —— 規格要求不跳 popup、不遮擋結果
- **公式完全未動**。改動後重跑房貸試算，仍為每月 29,570 元、總額 10,645,041 元，與改動前一致

**連帶修正 `rules-of-hooks` 違規**：`ToolsClient.tsx` 的純連結型 early return 位於兩個 `useState` 之前，若某計算器在「純連結」與「含輸入欄位」之間切換，hook 數量改變會導致白畫面。規格說既有問題「列出即可」，但規格同時寫明「功能穩定優先級永遠高於視覺效果」，且該檔本輪已在修改範圍內，故一併修掉。全站 lint 錯誤 9 → 7。

**FAQ（第二十節）**

答案原本是瞬間出現、無過場。改用 grid-rows `0fr → 1fr` 動畫，200ms 平滑展開且不需 JS 量測高度；答案恆存於 DOM，對搜尋引擎友善。另補 `aria-controls`、可見的 focus 樣式與 `motion-reduce` 處理。

**實用連結（第二十一節）**

空曠感的來源不是卡片，而是間隙：三個連續的 `key_value_list` 區塊各自帶垂直 padding，相鄰處疊加約 128px。`BlockRenderer` 本來就知道區塊順序，故讓「延續前一個同型別、同背景」的區塊取消上方 padding。頁面高度 1699 → **1571px**，正好等於兩處疊加間隙。

**驗證**：`BlockRenderer` 為全站共用，故重跑完整掃描 —— 9 頁 × 6 種寬度 = **54 組，水平溢出全部為 0**；欄位高度一致 48px、按鈕 52px；全站 10 條路由 200。

### P2：無障礙與效能

**方法：先稽核，再修改。** 撰寫檢查程式遍歷所有頁面，回報缺 alt 的圖片、無可讀名稱的連結／按鈕、未關聯 label 的表單欄位。

首次執行報 25 項，但其中 **9 項是我自己的 false positive** —— 頁尾 QR 連結內只有一個 `<img>`，而圖片的 alt 本身就會成為連結的可讀名稱，檢查邏輯未考慮。修正檢查後真實數量為 **16 項，全部在 `/tools`**。

**16 項皆為真實缺陷**：計算器的 `<label>` 沒有 `htmlFor`，input 也不在 label 內，螢幕閱讀器不會唸出欄位名稱。改為以 id 關聯，並以計算器 id 作為前綴（因不同計算器的 input id 會重複）。重新稽核：**9 頁全部 0 問題**。

**圖片效能**

`hero-bg.jpg` 原為 **3376×5063 直式照片（17 百萬像素、1.2MB）**，卻用作全寬橫幅背景，且它是 LCP 圖片。網站使用原生 `<img>`（有 `no-img-element` 豁免），靜態主機也沒有圖片處理管線，因此**原檔直接送給瀏覽器，無任何優化**。

7 張 JPEG 轉為 WebP q82，僅對超過 1920px 者縮圖以避免二次降質：

| 檔案 | 前 → 後 |
|---|---|
| hero-bg | 1196 KB → **167 KB（-86%）** |
| 總計 | 2702 KB → **787 KB（-71%）** |

轉換後以瀏覽器確認 hero 畫質，無可見差異。

**loading 屬性**：原本所有內容圖片都沒有。首屏圖片明確設為 `eager` + `fetchPriority="high"`（避免反而拖累 LCP），折線以下設 `lazy`。全站 6 eager／13 lazy，無遺漏。

**SEO 複查**（第二十九節要求檢查是否重複）：無重複 schema、每頁恰好 1 個 `h1`、title 各自不同、FAQPage 僅出現於 `/faq`。

**列出但未處理**：`public/defaults/certificate.jpg` 與 `license.jpg` 未被任何程式引用，共 567KB。**未刪除** —— #29 提到日後會加入證書，這兩張可能是預留，交由客戶決定。
...
| Issue | 待確認事項 |
|---|---|
| **#31** | **聯絡表單收件方式** —— 規格稱「使用 Google Form 嵌入」，查證為原生表單、無需修改；但靜態站無後端可收件，訪客送出的詢問會遺失。提出三方案，建議 Google Apps Script + Sheet。註記 Apps Script 的回應可能因 CORS 讀取不到，將先實測再採用，以免違反規格「不要建立假的送出成功畫面」 |
| **#32** | **服務 icon 改 SVG** —— 改後客戶無法自由輸入 emoji（後台目前可自選），需確認是否接受；替代方案為保留 emoji 但統一外框 |
| **#33** | **人物區突出哪 3～4 項資格** —— 屬專業判斷，請客戶指定 |
| #27 | 合作品牌 logo 素材（先前建立） |

### 尚未進行

P1 的 CTA 系統全站統一 —— 其中服務卡片部分取決於 #32（icon 是否改 SVG）的決定，先行處理可能白工。
